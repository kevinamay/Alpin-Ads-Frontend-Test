...
export function Rooms() {
  ...
  const CARD_W = isMobile ? 350 : 465;
  const GAP = 16;
  const STEP = CARD_W + GAP;
  const LEFT_PAD = isMobile ? 20 : 32;

  const goNext = () => { setAnimated(true); setTrackIdx((i) => i + 1); };
  const goPrev = () => { setAnimated(true); setTrackIdx((i) => i - 1); };

  const onTransitionEnd = () => {
    if (trackIdx >= total * 2) { setAnimated(false); setTrackIdx(trackIdx - total); }
    else if (trackIdx < total) { setAnimated(false); setTrackIdx(trackIdx + total); }
  };
        ...
        <div className="w-full max-w-[1440px] mx-auto flex flex-col items-center gap-[40px]">
          ...
          <div className="w-full px-[20px] md:px-[40px] flex flex-col items-center text-center gap-[12px]">
            <div className="w-full flex flex-col items-center gap-[8px]">
              ...
              <h2 className="w-full text-center text-[28px] md:text-[40px] font-normal text-[#323232] leading-[1.4] font-['Manrope']">
          ...
          <div className="w-full overflow-hidden">
            ...
            >
              {extendedRooms.map((room, idx) => (
                ...
                >
                  {/* Image */}
                  <div className="relative w-full" style={{ height: isMobile ? "240px" : "327px" }}>
                    ...
                    <div className="absolute top-[12px] right-[12px] bg-white rounded-[8px] px-[12px] py-[6px]">
                      <span className="font-['Manrope'] text-[14px] font-normal text-[#323232]">{room.price}</span>
                    </div>
                  </div>
                  {/* Card Body */}
                  <div className="flex flex-col p-[16px] gap-[16px]">
                    <div className="flex flex-col gap-[8px]">
                      <h3 className="text-[20px] font-normal text-[#323232] leading-[1.5] font-['Manrope']">
                        {room.name}
                      </h3>
                      <p className="text-[#323232]/70 leading-[1.5] font-['Manrope'] text-[14px] font-normal">
                        {room.desc}
                      </p>
                    </div>
                    <div className="flex flex-row items-center gap-[12px] w-full h-[24px]">
                      <div className="flex items-center gap-[8px]">
                        <Users className="w-[20px] h-[20px] text-[#323232]" strokeWidth={2} />
                        <span className="font-['Manrope'] font-normal text-[14px] text-[#323232]">{room.capacity}</span>
                      </div>
                      <div className="flex items-center gap-[8px]">
                        <MoveHorizontal className="w-[20px] h-[20px] text-[#323232]" strokeWidth={2} />
                        <span className="font-['Manrope'] font-normal text-[14px] text-[#323232]">{room.size}</span>
                      </div>
                    </div>
                    <button
                      onClick={() => setActiveRoom(room.popupData)}
                      className="w-full h-[48px] flex items-center justify-center gap-[10px] py-[12px] px-[32px] border border-[#323232]/20 rounded-[4px] backdrop-blur-[20px] hover:bg-black/5 transition-colors text-[#323232] font-['Manrope'] font-normal text-[14px] md:text-[16px] leading-[1.5] tracking-[0.05em] uppercase cursor-pointer"
                    >
                      SEE DETAILS
                    </button>
                  </div>
